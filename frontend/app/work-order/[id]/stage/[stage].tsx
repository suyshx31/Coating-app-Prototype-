...
export default function StageFormScreen() {
  ...
  const [startR, setStartR] = useState<Readings>(emptyR);
  const [endR, setEndR] = useState<Readings>(emptyR);
  ...
  const current = phase === "start" ? startR : endR;
  const setCurrent = (patch: Partial<Readings>) =>
    phase === "start" ? setStartR((s) => ({ ...s, ...patch })) : setEndR((s) => ({ ...s, ...patch }));

  // Gate check: surface temp must be > dew point + 3
  const gate = useMemo(() => {
    const st = n(current.surface_temp_c);
    const dp = n(current.dew_point_c);
  ...
  // DFT min/max validation
  const dftIssue = useMemo(() => {
    if (!wo) return null;
    const v = n(dft);
    if (v == null) return null;
    if (v > wo.spec.dft_max_um) return { hardBlock: true, msg: `DFT ${v} µm exceeds max ${wo.spec.dft_max_um} µm` };
    if (v < wo.spec.dft_min_um) return { hardBlock: false, msg: `Below min ${wo.spec.dft_min_um} µm` };
    return null;
  }, [dft, wo]);
  ...
  const allFilled = surfaceProfile && dft && salts && phase === "end" &&
    startR.surface_temp_c && endR.surface_temp_c;

  const canSubmit = !!allFilled && !dftIssue?.hardBlock && !submitting;
  ...
  const onSubmit = async () => {
    ...
    try {
      const body = {
        readings: {
          start: {
            ambient_temp_c: n(startR.ambient_temp_c),
            relative_humidity_pct: n(startR.relative_humidity_pct),
            dew_point_c: n(startR.dew_point_c),
            surface_temp_c: n(startR.surface_temp_c),
          },
          end: {
            ambient_temp_c: n(endR.ambient_temp_c),
            relative_humidity_pct: n(endR.relative_humidity_pct),
            dew_point_c: n(endR.dew_point_c),
            surface_temp_c: n(endR.surface_temp_c),
          },
        },
        parameters: {
          surface_profile_um: Number(surfaceProfile),
          dft_um: Number(dft),
          soluble_salts_mg_m2: Number(salts),
        },
        notes,
        photos,
        result: gate.ok && !profileIssue && !saltsIssue ? "pass" : "fail",
      };
      const r = await api.submitStage(wo.work_order_id, String(stage), body);
      router.replace(`/work-order/${wo.work_order_id}/submitted?stage=${stage}&result=${r.result}`);
    } catch (e: any) {
  ...
  const stageMeta = wo.stages.find((s) => s.key === stage);
        ...
        <ScrollView contentContainerStyle={{ padding: spacing.md, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
          ...
          <Card>
            ...
            <View style={styles.grid2}>
              <Field
                ...
                value={current.ambient_temp_c}
                onChangeText={(v) => setCurrent({ ambient_temp_c: v })}
              />
              <Field
                label="Humidity (%)"
                testID="env-rh-input"
                value={current.relative_humidity_pct}
                onChangeText={(v) => setCurrent({ relative_humidity_pct: v })}
              />
              <Field
                label="Dew Point (°C)"
                testID="env-dew-input"
                value={current.dew_point_c}
                onChangeText={(v) => setCurrent({ dew_point_c: v })}
              />
              <View style={{ flex: 1 }} />
            </View>
          </Card>
          ...
          <Card style={{ marginTop: spacing.md }}>
            ...
            <Field
              ...
              value={current.surface_temp_c}
              onChangeText={(v) => setCurrent({ surface_temp_c: v })}
          ...
          {/* Measured parameters (only on END phase makes sense, but allowed any time) */}
          <Card style={{ marginTop: spacing.md }}>
            ...
            <View style={{ marginTop: spacing.md }}>
              <View style={styles.rowBetween}>
                <Text style={[type.label, { color: colors.textPrimary }]}>DRY FILM THICKNESS (µm)</Text>
                <Text style={type.caption}>Min {wo.spec.dft_min_um} · Max {wo.spec.dft_max_um}</Text>
              </View>
              <TextInput
        ...
        <View style={styles.stickyBar}>
          <View style={styles.rowBetween}>
            <Text style={type.caption}>
              {phase === "start" ? "Complete START readings, then switch to END" : "Ready to submit"}
            </Text>
